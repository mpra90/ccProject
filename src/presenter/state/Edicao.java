...
import presenter.state.command.ComandoEdicao;

/**
 *
 * @author
 */
public class Edicao extends ManterClienteState {
    
    public Edicao(ManterClientePresenter manterCliente) {
        super(manterCliente);
        confTela();
        initListeners();
    }
    ...
    public void salvar() {
        if (manterCliente.validarCliente()) {
            ...
            manterCliente.getCliente().setNome(nome);
            manterCliente.getCliente().setIdade(idade);
            manterCliente.getCliente().setRenda(renda);
            
            new ComandoEdicao().executar(manterCliente);
